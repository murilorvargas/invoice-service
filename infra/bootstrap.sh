...
until docker compose exec localstack curl -sf http://localhost:4566/_localstack/health > /dev/null 2>&1; do
  echo "  LocalStack not ready yet, retrying..."
  sleep 2
done

echo "==> Configuring LocalStack..."
docker compose exec localstack awslocal sns create-topic --name card_entry
docker compose exec localstack awslocal sqs create-queue --queue-name invoice-card_entry-consumer
